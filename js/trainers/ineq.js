const INEQ = {
    ...
    resetScore() { this.score=0;this.total=0;this.streak=0; document.getElementById('ineq-score').textContent='0 / 0'; document.getElementById('ineq-pct').textContent='\u2014'; document.getElementById('ineq-streak').textContent='0'; },
    load() {
    ...
    record(ok) { this.total++; if(ok){this.score++;this.streak++;}else{this.streak=0;} document.getElementById('ineq-score').textContent=`${this.score} / ${this.total}`; document.getElementById('ineq-pct').textContent=this.total?Math.round(this.score/this.total*100)+'%':'\u2014'; document.getElementById('ineq-streak').textContent=this.streak; if(window.markAnswered)window.markAnswered(); },
