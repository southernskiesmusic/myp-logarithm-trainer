const RATIO = {
    ...
    resetScore() { this.score=0;this.total=0;this.streak=0; document.getElementById('ratio-score').textContent='0 / 0'; document.getElementById('ratio-pct').textContent='\u2014'; document.getElementById('ratio-streak').textContent='0'; },
    load() {
    ...
    record(ok) { this.total++; if(ok){this.score++;this.streak++;}else{this.streak=0;} document.getElementById('ratio-score').textContent=`${this.score} / ${this.total}`; document.getElementById('ratio-pct').textContent=this.total?Math.round(this.score/this.total*100)+'%':'\u2014'; document.getElementById('ratio-streak').textContent=this.streak; if(window.markAnswered)window.markAnswered(); },
