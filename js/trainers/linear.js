...
loadTrainerStats('linear',this);
    },
    next() { const p = this.level==='all' ? this.allPool : this.pools[this.level]; return pick(p)(); },
    resetScore() { this.score=0;this.total=0;this.streak=0; document.getElementById('linear-score').textContent='0 / 0'; document.getElementById('linear-pct').textContent='\u2014'; document.getElementById('linear-streak').textContent='0'; saveTrainerStats('linear',this); },
    load() {
    ...
    record(ok) { this.total++; if(ok){this.score++;this.streak++;}else{this.streak=0;} document.getElementById('linear-score').textContent=`${this.score} / ${this.total}`; document.getElementById('linear-pct').textContent=this.total?Math.round(this.score/this.total*100)+'%':'\u2014'; document.getElementById('linear-streak').textContent=this.streak; saveTrainerStats('linear',this); if(window.markAnswered)window.markAnswered(); },
