...
loadTrainerStats('trans',this);
    },
    next() { const p = this.level==='all' ? this.allPool : this.pools[this.level]; return pick(p)(); },
    resetScore() { this.score=0;this.total=0;this.streak=0; document.getElementById('trans-score').textContent='0 / 0'; document.getElementById('trans-pct').textContent='\u2014'; document.getElementById('trans-streak').textContent='0'; saveTrainerStats('trans',this); },
    load() {
    ...
    record(ok) { this.total++; if(ok){this.score++;this.streak++;}else{this.streak=0;} document.getElementById('trans-score').textContent=`${this.score} / ${this.total}`; document.getElementById('trans-pct').textContent=this.total?Math.round(this.score/this.total*100)+'%':'\u2014'; document.getElementById('trans-streak').textContent=this.streak; saveTrainerStats('trans',this); if(window.markAnswered)window.markAnswered(); },
